...
app.get('*', (req, res) => {
  ...
  console.log('context ===> ', require('util').inspect(context, { colors: true, depth: 2, showHidden: false }))

  renderer.renderToString(context, (err, html) => {
    console.log('html ===> ', require('util').inspect(html, { colors: true, depth: 2, showHidden: false }))
    if (err) {
      if (err.url) {
